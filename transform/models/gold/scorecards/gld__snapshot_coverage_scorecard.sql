...
--
-- Three count sources, joined per snapshot:
--   * fact_agg     — atomic counts + amount-kind coverage from the fact.
--   * bridge_agg   — comparable-code counts from the bridge.
--   * comparison_agg — tier + row-level blocker counts, re-derived from fact ⋈
--     bridge through the §6 framework macros (same classification the comparison
--     mart applies), so the scorecard explains blocked rows with the identical
--     vocabulary. Computed over ALL snapshots (not current-only) so non-current
--     snapshots are still scored. below_min_hospital_denominator is a mart-grain
--     (window) blocker and is intentionally not counted here.
--
-- Cross-snapshot aggregate → full-refresh table (config below) reading the fact and
-- bridge through plain ref(); excluded from the snapshot prune.

{{ config(materialized='table', incremental_strategy=none, tags=['gold_scorecards', 'gold']) }}
...
bridge_agg as (
    select
        ...
        count(distinct case
            when service_code_key is not null then silver_charge_item_id
        end) as items_with_comparable_code
    from {{ ref('gld_bridge__rate_observation_code') }}
    group by snapshot_id
),

-- Re-derive comparison tier + row-level blockers from fact ⋈ bridge using the §6
-- macros (same classification as gld__service_price_comparison_current, minus the
-- current-only filter and peer windows). Grain: observation × comparable cohort,
-- tier_0 collapse via the left join — identical to the mart.
...
select
    ...
    -- code coverage
    b.distinct_comparable_codes,
    b.items_with_comparable_code,
    b.items_with_comparable_code / nullif(f.charge_item_count, 0)::double
