...
--
-- hpt_csv_code_unpivot collapses its input to charge grain before unpivoting, so
-- the payer-copy fanout (CSV-wide Bronze repeats charge-level code columns once
-- per payer row) is handled inside the macro -- this model just hands it the raw
-- staged rows. See the macro and docs/cleanup.md.
